...
writer.writerow([
    "Time (s)", "Accel_x (m/s2)", "Accel_y (m/s2)", "Accel_z (m/s2)",
    "RPM", "Humidity (%)", "Amb_Temp (C)", "Water_Temp (C)",
    "Motor_Temp (C)", "noBond_height (mm)", "Bond_height (mm)"
])
...
def Update_graphs():
    ...
    global ser, writer, time_start

    
    # Abre el archivo CSV en modo de escritura
    try:
        if Bond_wave_counter < max_waves:
            if not ard_data_queue.empty():
                ...
                # Ask for total number of crests
                    ...
                    if crest_flag:
                        ...
                        #winsound.Beep(350,500)
                        print('ambient humdiity line:', line)
    ...
    window.after(500, Update_graphs)              
