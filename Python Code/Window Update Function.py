...
# Variables globales para el VFD
vfd_client = None
vfd_connected = False
vfd_lock = threading.Lock()

def inicializar_vfd():
    """Ejecutar Modbus en un hilo separado para no bloquear"""
    global vfd_client, vfd_connected
    try:
        print("Inicializando VFD en segundo plano...")
        ...
        if client.connect():
            ...
            print("✅ VFD conectado")
            # Escribir frecuencia
            client.write_register(0x0002, 1800, device_id=1)
            print("✅ Frecuencia configurada a 18 Hz")
            return True
        else:
            print("❌ No se pudo conectar al VFD")
            return False
...
def encender_motor():
    """Enciende el motor usando el cliente VFD"""
    global vfd_client, vfd_connected
    try:
        with vfd_lock:
            if vfd_connected and vfd_client:
                print("Encendiendo motor...")
                vfd_client.write_register(0x0001, 1, device_id=1)
                print("✅ Motor encendido")
                return True
            else:
                print("❌ VFD no está conectado")
                return False
    except Exception as e:
        print(f"❌ Error encendiendo motor: {e}")
        return False

threading.Thread(target=inicializar_vfd, daemon=True).start()
    ...
    with open(csv_filename, mode='w', newline='') as file:
        ...
        try:
            ...
            while Bond_wave_counter < max_waves:
                ...
                #try:
                    # ser = serial.Serial(ard_port, boudrate, timeout=0.5)
                 #   print("loop0")
                  #  if ser.in_waiting > 0:
                   #     ser_available = True
                #except:
                ...
                if  ser.in_waiting > 0:
                    ...
                    # --- START MOTOR ---
                        ...
                        threading.Thread(target=encender_motor, daemon=True).start()  # Vuelve INMEDIATAMENTE
