...
void Zero_Leveling() {
  ...
  while (true) {
    ...
    // Print out values to serial to observe results
    Serial.print("Zero levels,"); // Coms code
...
void Inductive() {
  ...
  memset(average_velocities, 0, sizeof(average_velocities));
  memset(average_velocities_sorted, 0, sizeof(average_velocities_sorted));
  final_average_velocity = 0;
  average_velocities_counter = 0;

  unsigned long startTime = millis();

  // Read first couple of meassurements to ignore first high meassurements if sensor was reading when 
  // code started
  while (true){
    // Check for timeout
    ...
    if (digitalRead(ind_pin) == HIGH) { // LOW for infrared, HIGH for inductive
      ...
      // Any subsequent pulse
        ...
        // Out of bounds
          ...
          // Calculates the median of the calculated average RPM 
          if (average_velocities_counter == angular_velocity_checks){
            ...
            rpm_measured = true;
            Serial.print("Final average angular velocity: ,");
            Serial.println(ind_avg_freq);
            break;
      ...
      delay(300); // Delay to multiple meassurements in the same pass
    }
...
void All_Measurements() {
  ...
  */
  ...
  // THERMISTORS
  //Lectura del Arduino
  SensorValue_W = 500; // Valor actual de agua fijo ya que no se tiene el sensor puesto
...
void CSV_Results() {
  ...
  // Time
  Serial.print("Wave data"); // Coms code
  Serial.print(",");
...
void loop() {
  ...
  // Inicia proceso al recibir un 1 desde la comunicacion serial
  if (!zero_leveled && bandera_inicio && !water_moving){
    Zero_Leveling();
